...
class Construct(object):
    def __init__(self, model, loss_fn, optimizer, decay_rate=0):
        # Here we define the attributed of our class
        ...
        # Note: there are NO ARGS here! It makes use of the class
        # attributes directly
    ...
    def _make_train_step_fn(self):
        ...
        def perform_train_step_fn(x, y):
            ...
            reg_norm = sum(p.abs().sum() for p in self.model.parameters()) # L1
            # reg_norm = sum((p**2).sum() for p in self.model.parameters()) # L2
            loss = loss + self.reg_lambda * reg_norm
    ...
    def _make_val_step_fn(self):
        ...
        def perform_val_step_fn(x, y):
            ...
            # Step 2 - Computes the loss
            loss = self.loss_fn(yhat, y)
            # There is no need to compute Steps 3 and 4, since we do not update parameters during evaluation
            return loss.item()
    ...
    def train(self, n_epochs, seed=42):
        ...
        for epoch in range(n_epochs):
            ...
            # print statistics
            print('[Epoch %d] loss: %.6f | %% loss: %.1f%%' % (epoch, loss, loss/self.losses[0]*100))
    ...
    def predict(self, x):
        # Sets to evaluation mode for predictions
        self.model.eval()
        # Takes a numpy input and makes it a float tensor
        x_tensor = torch.as_tensor(x).float()
        # Sends the input to device and uses model for prediction
        y_hat_tensor = self.model(x_tensor.to(self.device))
        # Sets it back to train mode
        self.model.train()
    ...
    def make_normalizer(loader):
        total_samples, total_means, total_stds = Construct.loader_apply(loader, Construct.statistics_per_channel)
